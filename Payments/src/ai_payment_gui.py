...
from ai_payment_db import save_single_prediction, save_batch_predictions, engine,mark_refunded 
...
if st.session_state['active_panel'] == "Payment":
    ...
    if submitted:
        ...
        if txn_id:
            # ✅ Include amount in session state so Refund panel works
            res["amount"] = transaction["amount"]
            st.session_state['transactions'][txn_id] = res

            save_single_prediction(
                txn_id,
                res.get("decision", "Rejected ❌"),
                res.get("fraud_prob", 1.0),
        ...
        if not res.get("valid", True):
            st.markdown(
                f'<div class="error-card">{res.get("reason", "Invalid payment details")}</div>',
                unsafe_allow_html=True
            )
        else:
            ...
            color_class = (
                "success-card" if decision == "Approved ✅"
                else ("warning-card" if decision == "Flagged ⚠️" else "error-card")
            )

            # ✅ Show the entered amount in the AI decision message
            st.markdown(
                f'<div class="{color_class}">'
                f'AI Decision: {decision} | Fraud Probability: {fraud_prob:.2f} | Amount: ${entered_amount:.2f}'
                f'</div>',
                unsafe_allow_html=True,
            )
...
# ---------------------------
    ...
    if refund_submit:
        ...
        if txn_record is None:
            ...
            amt = txn_record.get("amount", 0.0)

            # Step 1: update session state
            success = process_refund(refund_txn, st.session_state['transactions'])
            st.session_state['transactions'][refund_txn]['refunded_amount'] = amt

            # Step 2: update database
            db_success = mark_refunded(refund_txn, amount_refunded=amt)
            if not db_success:
                success = False
                msg = f"Refund failed: could not update DB for transaction {refund_txn}."
            else:
                msg = f"Refund processed for {refund_txn} | Amount Refunded: ${float(amt):.2f}"

        st.markdown(f'<div class="{"success-card" if success else "error-card"}">{msg}</div>', unsafe_allow_html=True)

# ---------------------------
# Batch Processing panel (Admin)
# ---------------------------
